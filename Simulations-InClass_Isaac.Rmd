...
```{r}
boot_ci<-function(data,funct=data.sample){
  samp<-sample(data,length(data),replace=TRUE)
  samp.stat<-funct(samp)
  vector.stat<-replicate(1000,samp.stat)
  low.bound<-quantile(vector.stat,probs=0.025)
  high.bound<-quantile(vector.stat,probs=0.975)
